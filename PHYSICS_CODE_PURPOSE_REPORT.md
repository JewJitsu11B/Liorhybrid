...
- **Bayesian collapse with prior memory** (`kernels/bayesian.py`, `kernels/fractional_memory.py`): evidence enters via a Λ_QR term while history contributes only as a scalar weight, matching Bayes' rule $P(T \mid D) \propto P(D \mid T) P(T_{\text{prior}})$ with $T_{\text{prior}}$ denoting the prior field state.
...
- **Three-component LIoR memory kernel** (`models/lior_kernel.py`): fast exponential decay, long-tail power-law memory, and oscillatory phase terms together approximate long-range memory with a finite recurrence while enforcing stability ($\rho \in (0,1)$).

## Where modern ML architecture was replaced to satisfy the physics
- **State-space core:** Standard linear Mamba transition $h_t = A h_{t-1} + B x_t$ is replaced by **CI8 Trinor evolution** (`inference/geometric_mamba.py`). The Trinor is the geometric-algebra analogue of the transition matrix: a learned rotation/scale operator in complex octonion space that preserves causal orientation while remaining $O(N)$.
...
- **Training loop & metrics:** `training/lior_trainer.py`, `training/metrics.py`
...
4. **Learning signal:** Geodesic and entropy terms penalize paths that deviate from the field manifold. Gradients stay bounded because the metric is built from the field then gradient-detached (no backprop through it). Dimensional consistency is preserved because the metric matches the field tensor’s dimension $D$.
