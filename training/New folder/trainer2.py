...
def run_window(
    ...
    with inference_context():
        ...
        for _t in range(steps):
            if progress_every > 0 and (_t % progress_every == 0):
                # Minimal host-side progress marker (no cuda sync)
                print(f"[trainer2]   step {_t}/{steps}", flush=True)
        ...
        if cfg.timing_debug and ev1 is not None and ev0 is not None:
            ...
            print(f"[trainer2] window gpu_ms={ms:.2f}", flush=True)
