...
from .biquaternion import BiQuatCausalLayer, pack_biquat, unpack_biquat
...
class BiQuatCausalBlock(nn.Module):
    """
    Causal field block using biquaternion algebra.

    Replaces cubic O(d^3) operations with O(d^2) structured matmuls.
    ...
    def __init__(
        ...
        # Biquaternion causal layer (O(d^2) instead of O(d^3))
        self.field_layer = BiQuatCausalLayer(
            d_model=d_model,
            d_field=d_field,
            alpha=alpha,
            use_fft_memory=use_fft_memory
        )
    ...
    def forward(
        ...
        Args:
            ...
            memory: Previous memory state
            attention_mask: Optional attention mask
            diagnose: Unused (for API compatibility)
        ...
        x, new_memory = self.field_layer(x, memory)
